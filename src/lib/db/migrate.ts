...
export function applyMigrations(db: Database.Database): void {
  ...
  // Phase 1 → Phase 2: graph_status + level_pin on notes
  {
    ...
    if (cols.size > 0) {
      ...
      // Phase 5 → Phase 6: bilingual companion columns
      if (!cols.has("has_en")) {
        db.exec("ALTER TABLE notes ADD COLUMN has_en INTEGER NOT NULL DEFAULT 0")
      }
      if (!cols.has("body_en")) {
        db.exec("ALTER TABLE notes ADD COLUMN body_en TEXT")
      }
      ...
      // Phase 7 → Phase 8: related column (frontmatter explicit links)
      if (!cols.has("related")) {
        db.exec("ALTER TABLE notes ADD COLUMN related TEXT NOT NULL DEFAULT '[]'")
      }
    }
  }
