...
에이전트 생태계의 다음 병목은 모델 성능보다 **interoperability**일 가능성이 높다. 사내 에이전트가 외부 서비스 에이전트를 호출하고, 그 결과를 다시 또 다른 에이전트로 넘기는 다단 흐름이 일상이 되면, 매번 새로 짜는 어댑터가 시스템 전체의 발목을 잡는다. MCP가 "에이전트 ↔ 도구" 인터페이스를 표준화한 것처럼, A2A는 "에이전트 ↔ 에이전트" 인터페이스를 표준화한다. 둘은 경쟁이 아니라 보완 관계로 읽힌다.

특히 인상적인 부분은 **Task의 ID 중심성**이다. 통신 패턴은 polling이든 SSE든 push든 자유롭게 갈아탈 수 있지만, Task ID는 그 모든 패턴을 관통하는 단일 정체성이다. 분산 시스템에서 작업 추적과 재시도, 재개를 가능하게 하는 가장 작은 단위가 잘 잡혀 있다.

남는 질문은 **신뢰 모델**이다. Agent Card에 선언된 capability와 skill을 Client가 그대로 믿어도 되는가, 악의적 Server가 사실과 다른 Agent Card를 노출하면 어떻게 막는가. 인증은 정의되어 있지만, 평판·검증·감사(audit) 계층은 스펙 바깥에 남아 있다. 
