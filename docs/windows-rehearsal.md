...
Drives the 8 steps below — automates the mechanical parts (prereq check, clean fork, scripted `/init`, push, `publish-verify`, daily-audit trigger) and stops at 4 visual checkpoints where you confirm browser-visible results with `[y/N]`. Produces `docs/rehearsals/YYYY-MM-DD-windows.md` with PASS/FAIL per step. The script is a convenience; the manual walkthrough below is still the source of truth and catches anything the script skips.

Use `-SkipPush` to skip criteria #6 + #7 (no throwaway fork repo on GitHub).
