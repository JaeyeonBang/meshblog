...
VALUES
  ...
  ('fixture-sqlite-patterns',
   ...
   'better-sqlite3 is a synchronous SQLite driver for Node. Key patterns: WAL mode for concurrent reads, prepared statements for performance, transactions for batch writes, and PRAGMA foreign_keys=ON. Prepared-statement types pair well with [[fixture-ts-generics|TypeScript generics]], and the [[Driver Playbook]] covers the rough edges.',
  ...
  ('fixture-rag-overview',
   ...
   'RAG combines a vector store with an LLM. At query time, embed the question, retrieve top-k chunks, then pass them as context to the model. OpenAI text-embedding-3-small produces 1536-dim vectors. For persistence use something embedded and local — see [[Better-SQLite3 Patterns]].',
